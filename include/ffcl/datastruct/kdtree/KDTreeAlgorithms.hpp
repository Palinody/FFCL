...
ssize_t select_axis_with_largest_variance(const IndicesIterator& indices_range_first,
    ...
    if (n_choices > 2) {
        const auto random_indices =
            math::random::select_n_random_indices_from_indices(indices_range_first, indices_range_last, n_choices);
    ...
    if (n_samples > 2) {
        const auto random_indices =
            math::random::select_n_random_indices_from_indices(indices_range_first, indices_range_last, 3);
        // return the feature index with the maximum variance
        return math::statistics::argmax_variance_per_feature(
            random_indices.begin(), random_indices.end(), samples_range_first, samples_range_last, n_features);
    }
...
ssize_t select_axis_with_largest_variance(const IndicesIterator&          indices_range_first,
    ...
    if (n_choices > 2) {
        const auto random_indices =
            math::random::select_n_random_indices_from_indices(indices_range_first, indices_range_last, n_choices);
        // return the feature index with the maximum variance
        return math::statistics::argmax_variance_per_feature(random_indices.begin(),
                                                             random_indices.end(),
                                                             samples_range_first,
                                                             samples_range_last,
                                                             n_features,
...
shift_median_to_leftmost_equal_value(std::size_t                                   median_index,
    ...
    while (std::distance(left_indices_range.first, left_neighbor_value_it) >= 0 &&
           common::utils::equality(samples_range_first[left_neighbor_value_it[0] * n_features + feature_index],
                                   cut_value)) {
...
quickselect_median(const IndicesIterator& indices_range_first,
    ...
    const auto median_indices_range = ffcl::algorithms::quickselect(indices_range_first,
                                                                    indices_range_last,
                                                                    samples_range_first,
                                                                    samples_range_last,
                                                                    n_features,
