...
auto dual_set_shortest_edge_distance(
    ...
    const typename std::iterator_traits<SamplesIterator>::value_type& initial_shortest_edge_distance =
    ...
    auto shortest_edge_distance = initial_shortest_edge_distance;
    ...
    for (auto index_it = indices_range_first; index_it != indices_range_last; ++index_it) {
        for (auto other_index_it = other_indices_range_first; other_index_it != other_indices_range_last;
            ...
            if (samples_distance > 0 && samples_distance < shortest_edge_distance) {
                shortest_edge_distance = samples_distance;
            }
        }
    }
...
template <typename IndicesIterator,
          ...
          typename OtherIndicesIterator,
          typename OtherSamplesIterator>
auto dual_set_shortest_edge_distance_with_union_find(
    const IndicesIterator&                                            indices_range_first,
    const IndicesIterator&                                            indices_range_last,
    const SamplesIterator&                                            samples_range_first,
    const SamplesIterator&                                            samples_range_last,
